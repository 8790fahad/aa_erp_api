/**
 * Clean deposit + open credit (A/R) for facility retest.
...
async function main() {
  const beforeDep = await db.sequelize.query(
    `SELECT transaction_ref AS customer_no, ROUND(SUM(cr)-SUM(dr),2) AS bal
     FROM general_ledger
     WHERE facility_id = :f AND LOWER(COALESCE(type,'')) = 'deposit'
     GROUP BY transaction_ref HAVING ABS(bal) > 0.01`,
    { replacements: { f: FACILITY }, type: QueryTypes.SELECT },
  );
  console.log("Deposits BEFORE:", beforeDep);
  ...
  try {
    // 1) Wipe all deposit GL + related entries
    const [depGl] = await db.sequelize.query(
      `DELETE FROM general_ledger
       WHERE facility_id = :f AND LOWER(COALESCE(type,'')) = 'deposit'`,
      { replacements: { f: FACILITY }, transaction: t },
    );

    const [aaGl] = await db.sequelize.query(
      `DELETE FROM general_ledger
       WHERE facility_id = :f
         AND (
           reference_number LIKE 'AA-%'
           OR purpose_of_payment LIKE 'Apply customer advance%'
           OR transaction_description LIKE '%AUTOTEST%'
      ...
      `DELETE FROM customer_entries
       WHERE facilityId = :f
         AND (
           ...
           OR description LIKE 'TEST CLEAN%'
         )`,
      { replacements: { f: FACILITY }, transaction: t },
    );
    ...
    const [workflows] = await db.sequelize.query(
      `DELETE FROM sale_workflows
       WHERE facility_id = :f
         AND (sale_code LIKE 'TEST-DEP-%' OR notes LIKE '%AUTOTEST%')`,
      { replacements: { f: FACILITY }, transaction: t },
    );

    // 2) Clear open invoice AR with balancing CR (retest clean)
    let cleared = 0;
    ...
    // Cancel open credit workflows so they don't sit in VP queues mid-test
    const openRefs = open.map((r) => r.invoice_ref).filter(Boolean);
    if (openRefs.length) {
      await db.sequelize.query(
        ...
        {
          replacements: { f: FACILITY, uid: USER_ID, refs: openRefs },
          transaction: t,
        },
      );
    }
    ...
    await t.commit();
    console.log("Deleted deposit GL:", depGl?.affectedRows ?? depGl);
    console.log("Deleted AA/TEST CLEAN GL:", aaGl?.affectedRows ?? aaGl);
    ...
    console.log("Cleared open AR invoices:", cleared);
  } catch (err) {
    await t.rollback();
    throw err;
  }

  const afterDep = await db.sequelize.query(
    `SELECT transaction_ref AS customer_no, ROUND(SUM(cr)-SUM(dr),2) AS bal
     FROM general_ledger
     WHERE facility_id = :f AND LOWER(COALESCE(type,'')) = 'deposit'
     GROUP BY transaction_ref HAVING ABS(bal) > 0.01`,
  ...
  const afterCredit = await db.sequelize.query(
  ...
  console.log("Deposits AFTER:", afterDep);
  console.log("Credit AFTER:", afterCredit);
  await db.sequelize.close();
  console.log("DONE — refresh Received Payment to retest.");
}
